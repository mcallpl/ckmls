<?php
/**
 * Quick CMA Email Generator
 * All agent info comes from config.php — nothing hardcoded here.
 */
...
if (empty($rawInput)) {
    echo json_encode(['success'=>false,'error'=>'Empty request body. Filter to fewer properties and try again.']); exit;
}
...
if (!$previewOnly && empty($recipients)) {
    echo json_encode(['success'=>false,'error'=>'Please enter at least one recipient email.']); exit;
}
if (empty($properties)) {
    echo json_encode(['success'=>false,'error'=>'No properties received. Please try again.']); exit;
}
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;box-shadow:0 4px 16px rgba(0,0,0,.08);">
        ...
        <tr><td style="padding:16px 18px;background:#fff;">
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:10px;"><tr>
                <td><span style="display:inline-block;padding:3px 10px;border-radius:20px;background:'.$sc['bg'].';color:'.$sc['text'].';font-size:10px;font-weight:700;text-transform:uppercase;">'.$status.'</span>
                '.($dist ? '<span style="font-size:10px;color:#9ca3af;margin-left:8px;">'.$dist.'</span>' : '').'</td>
...
// Social icons — only those defined
$socialHtml = '';
$socialLinks = [
...
<title>Quick CMA &mdash; '.htmlspecialchars($subjectAddr).'</title>
</head>
...
<tr><td style="border-radius:16px 16px 0 0;background:linear-gradient(135deg,#1e1b4b 0%,#312e81 50%,#1e3a5f 100%);padding:40px 40px 36px;">
    <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.18em;color:#a5b4fc;margin-bottom:12px;">Quick Comparative Market Analysis</div>
...
<tr><td style="background:#fff;padding:36px 40px;">
    ...
    <div style="border-bottom:2px solid #e5e7eb;padding-bottom:12px;margin-bottom:24px;">
        <h2 style="margin:0;font-size:16px;font-weight:700;color:#111827;text-transform:uppercase;letter-spacing:.08em;">Comps In &amp; Around the Subject Property</h2>
    </div>
    '.$propRows.'
    ...
    <div style="text-align:center;padding:24px;background:#f8f7ff;border-radius:12px;">
        ...
        <p style="margin:0 0 20px;font-size:13px;color:#6b7280;line-height:1.7;">This Quick CMA is a snapshot. For a full analysis including pricing strategy and a personalized consultation, reach out anytime.</p>
        '.$ctaHtml.'
    </div>
</td></tr>
...
<tr><td style="background:#1f2130;border-radius:0 0 16px 16px;padding:20px 40px;">
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
        <td><div style="font-size:10px;color:#8892a4;line-height:1.6;">This Quick CMA is based on MLS data and public records. All information is deemed reliable but not guaranteed. This is not an appraisal. Contact your agent for a full evaluation.</div></td>
...
$errors  = [];
$sent    = [];
$subject = $emailSubject ?: "Your Quick CMA — {$subjectAddr}";
...
foreach ($recipients as $r) {
    ...
    if ($toName) {
        $html = str_replace(
            '<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:#6366f1;margin-bottom:14px;">Market Analysis</div>',
            '<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:#6366f1;margin-bottom:14px;">Market Analysis</div><p style="margin:0 0 14px;font-size:15px;font-weight:600;color:#1e1b4b;">Hi '.htmlspecialchars($toName).',</p>',
            $html
        );
    }

    // Use exact address GoDaddy authenticated — matches working email from Mar 18
...
echo json_encode([
    ...
    'message' => count($sent) > 0
        ? 'Quick CMA sent to '.implode(', ',$sent).(count($errors) ? ' ('.implode('; ',$errors).')' : '')
